...
class Trainer:
    def __init__(self, options):
        ...
        #self.create_segmentation_masks()
        ...
        self.models["segmentation"] = networks.Unet(
        ...
        self.segmentation_parameters = list(self.models["segmentation"].parameters())
        ...
        self.segmentation_optimizer = optim.Adam(self.segmentation_parameters, self.opt.segmentation_learning_rate)

        self.model_lr_scheduler = optim.lr_scheduler.StepLR(
            self.model_optimizer, self.opt.scheduler_step_size, 0.1)
        self.segmentation_lr_scheduler = optim.lr_scheduler.StepLR(
            self.segmentation_optimizer, self.opt.scheduler_step_size, 0.1)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            self.segmentation_optimizer.zero_grad()
            #losses["segmentation"].backward()
    ...
    def process_batch(self, inputs):
        ...
        outputs["segmentation"] = self.models["segmentation"](outputs[("disp", 0)])
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if scale == 1:
                segmentation_loss = self.get_segmentation_loss(outputs["segmentation"], inputs["segmentation_gt"])
    ...
    def get_segmentation_loss(self, logits, target):
        ...
        weighted_logp = (logp * weights).view(dims[0], -1)

        # Rescale so that loss is in approx. same interval
        weighted_loss = weighted_logp.sum(1) / weights.view(dims[0], -1).sum(1)
    ...
    def save_model(self):
        ...
        save_path = os.path.join(save_folder, "{}.pth".format("adam_segmentation"))
        torch.save(self.segmentation_optimizer.state_dict(), save_path)
    ...
    def load_model(self):
        ...
        # loading segmentation adam state
